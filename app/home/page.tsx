...
// Importing the neccesary modules for the seach bar page
...
function InputBar({ setInputText }: inputBarProps) {
  return (
    <input
      ...
      className={`w-full h-14 rounded-2xl bg-white/5 text-white placeholder:text-white/40 border border-white/10 pl-12 pr-4 text-[15px] ${poppins.className} shadow-lg backdrop-blur outline-none transition focus:border-emerald-400/40 focus:ring-4 focus:ring-emerald-400/15`}
      onChange={(e) => setInputText(e.target.value)}
    />
  );
}

// To find out players from total list of players with user's input
...
// The component for displaying the found players
