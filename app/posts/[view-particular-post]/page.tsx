...
export default function ParticularPost() {
	...
	const commentsData = Comments.filter((comment) => comment.postId === post?.id);
			...
			<div className="max-w-3xl mx-auto flex flex-col h-full gap-1">
				...
				<section className="mt-5 w-full flex-1 min-h-0 flex flex-col px-3 relative">
					<div className="flex items-center justify-between pb-3 px-1 border-b border-white/10">
						<h2 className={`${oswald.className} text-md text-white font-semibold`}>
							COMMENTS
						</h2>
						...
						{commentsData.length > 0 ? (
							commentsData.map((comment) => {
								...
								const commentMentions = comment.mentions.map((mentionId) => {
									const mentionedUser = users.find((item) => item.id === mentionId);
								...
								return (
									<div
										key={comment.id}
										className="relative px-2 py-4 md:px-3 flex items-start justify-between gap-3 border-b border-white/20 last:border-b-0"
									>
										<div className="flex items-start gap-3 flex-1 min-w-0">
											...
											<div className="min-w-0 flex flex-col gap-2">
												<div className="flex items-center gap-2 flex-wrap">
													<p className={`${poppins.className} text-sm text-white font-medium truncate`}>
														{commentAuthor?.name ?? "Unknown user"}
													</p>
													<p className={`${poppins.className} text-xs text-white/60 truncate`}>
														@{commentAuthor?.username ?? "unknown"}
													</p>
													<span className={`${poppins.className} text-[11px] text-white/45`}>
													...
													{validMentions.length > 0 && (
														<p className="text-xs leading-5 mb-0.5">
															{validMentions.map((mention) => (
																<span key={mention} className="text-emerald-300 mr-1">
																	@{mention}
																</span>
															))}
														</p>
										...
										<button
											type="button"
											className="shrink-0 flex flex-col items-center justify-center gap-0.5 px-2 py-1 rounded-md hover:bg-white/10 transition-colors h-16"
										>
					...
					<div className="fixed bottom-20 left-0 right-0 z-40 px-4 md:px-6 py-3">
						<div className="max-w-3xl mx-auto px-3">
							<div className="w-full flex items-center gap-3 rounded-xl border border-white/40 bg-[#0B1323]/95 backdrop-blur px-4 py-2 shadow-[0_6px_18px_rgba(0,0,0,0.35)]">
								<input
